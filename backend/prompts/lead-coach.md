...
- `score` (numero inteiro 0-100): nota interna de potencial/temperatura do lead com base no contexto (dor, intencao, fit, sinais de compra). Use `null` se nao houver base no contexto — NUNCA chute um valor.
- `previa_valor` (objeto): sugestao INTERNA de valor para o projeto deste lead (uso do operador, NUNCA vai ao lead), ancorada nas faixas reais: iniciante R$200-600, padrao R$600-1000, premium R$1000-2000. Para sistema, automacao, agente de IA ou projeto sob medida use `plano: "sob_medida"` e faixas null. Campos: `{ "faixa_min": numero|null, "faixa_max": numero|null, "valor_alvo": numero|null, "plano": "iniciante"|"padrao"|"premium"|"sob_medida", "justificativa": string curta }`. Estime uma FAIXA pelo contexto (nicho, cidade, dor) quando faltar diagnostico; use `null` se nao der pra estimar com seguranca.
- `analise_detalhada` (objeto): `{ "valor_percebido": array de strings, "dores": array de strings, "razoes_compra": array de strings, "objecoes": array de strings }`. Use SOMENTE o que aparece na conversa/perfil deste lead; deixe arrays vazios quando nao houver evidencia. NUNCA invente itens genericos (ex.: nao escreva "competir com outros barbeiros" se o lead nao e barbeiro).
