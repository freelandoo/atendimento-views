...
- Atribuição Meta Ads (CTWA) em `src/services/meta-attribution.js`, chamada a cada ~10 min pelo worker (`sincronizarAtribuicaoMetaAds`): captura de qual anúncio cada lead veio → grava `vendas.lead_profiles.origem='meta_ads'` + `origem_anuncio` jsonb `{ad_id, ctwa_clid, title, source_url}`; e recalcula `score_lead` por CRITÉRIOS (determinístico, `calcularScoreLeadDeterministico`) p/ leads ativos. **Este módulo NÃO envia nada à Meta** — quem envia é a integração por empresa (bloco abaixo).
- ⚠️ **A captura de CTWA nunca funcionou, e não é por falta de dado.** Medido em produção (2026-08-08), em três camadas independentes:
  1. **Schema errado.** O código procura `public."Message"`; a tabela do Evolution é `evolution."Message"` — **mesmo banco**, outro schema (`public` está vazio). `to_regclass` devolve null e a função vira **no-op silencioso** a cada tick. Não existe "banco separado do Evolution": esse diagnóstico era errado.
  2. **`key.remoteJidAlt` não existe** nesta versão: a coluna `key` guarda só `{fromMe, id, remoteJid}`. É justamente o campo de onde o código tira o telefone.
  3. **O lead de anúncio chega como `@lid`, não como telefone.** Das 526 mensagens com `externalAdReply` (509 com `ctwaClid`, 18 anúncios distintos), **100%** têm `remoteJid` `@lid` — identificador opaco. O filtro `LIKE '%@s.whatsapp.net'` descarta todas. E **não há tradução `@lid` → telefone** em `Contact`/`Chat`: 251 telefones de anúncio, **0** casam com `vendas.conversas`.
  **Onde o dado existe junto:** no **webhook**. `vendas.conversas.numero` é `@s.whatsapp.net` em 100% das 62 conversas — ou seja, o payload traz o telefone que o banco do Evolution não persiste (`canonicoRemoteJidParaConversa`, e o aviso já plantado em `webhook-handler.js:83`). Qualquer retomada de CTWA deve **capturar no webhook**, não minerar `evolution."Message"`. Falta confirmar ao vivo que `externalAdReply` chega no payload — é a única suposição pendente.
