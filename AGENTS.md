...
- **Selo de WhatsApp por lead** (`prospectador.prospects.tem_whatsapp`, migration `021`): aprendido no disparo e também pelo worker `src/services/whatsapp-verificacao-worker.js` — envio OK ou Evolution `/chat/whatsappNumbers` `exists:true` ⇒ `true` (ícone verde no painel); Evolution `exists:false`/`numero_inexistente` ⇒ `false` (registra "sem WhatsApp", sai da elegibilidade). `NULL` = ainda não verificado. A verificação passiva **não envia mensagem** e não usa foto de perfil como negativo.
- `BANCO_LEADS_AUTO_WORKER_MS` (default `60000`, mín `30000`): intervalo do tick do worker do modo Automático.
- `WHATSAPP_VERIFICACAO_WORKER_MS` (default `300000`, mín `30000`) / `WHATSAPP_VERIFICACAO_BATCH` (default `20`, 1..50): intervalo e lote do worker que separa leads pendentes entre "com WhatsApp" e "sem WhatsApp" usando a Evolution, sem disparar mensagem.
